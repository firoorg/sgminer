 ...
 */
void getargon_blockIndex(uint32_t ij, argon2_instance_t *instance, uint32_t *Index)
{
	uint32_t ij_prev = 0;
...
truc++;
        ...
        if (data_independent_addressing) {
            if (i % ARGON2_ADDRESSES_IN_argon_block == 0) {
                next_addresses(&address_argon_block, &input_argon_block, &zero_argon_block, (uint32_t*)instance->argon_block_header);
            }
            pseudo_rand = address_argon_block.v[i % ARGON2_ADDRESSES_IN_argon_block];
        } else {
