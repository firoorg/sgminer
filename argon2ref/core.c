...
void finalize(const argon2_context *context, argon2_instance_t *instance) {
    if (context != NULL && instance != NULL) {
        ...
        /* Hash the result */
        {
            ...
            ablake2b_long(context->out, context->outlen, argon_blockhash_bytes,ARGON2_argon_block_SIZE);
...
static int fill_memory_argon_blocks_st(argon2_instance_t *instance) {
    uint32_t r, s, l;
...
static int fill_memory_argon_blocks_mt(argon2_instance_t *instance) {
    uint32_t r, s;
...
int fill_memory_argon_blocks(argon2_instance_t *instance) {
...
int fill_memory_argon_blocks_mtp(argon2_instance_t *instance) {
	uint32_t r, s;
	argon2_thread_handle_t *thread = NULL;
	argon2_thread_data *thr_data = NULL;
	int rc = ARGON2_OK;
	...
	// fail to fill argon_blocks with argon2d
fail:
	if (thread != NULL) {
		free(thread);
	}
...
void fill_first_argon_blocks(uint8_t *argon_blockhash, const argon2_instance_t *instance) {
    uint32_t l;
    ...
    for (l = 0; l < instance->lanes; ++l) {
        ...
        store32(argon_blockhash + ARGON2_PREHASH_DIGEST_LENGTH, 0);
        store32(argon_blockhash + ARGON2_PREHASH_DIGEST_LENGTH + 4, l);

        ablake2b_long(argon_blockhash_bytes, ARGON2_argon_block_SIZE, argon_blockhash,
    ...
    //               argon_blockhash_bytes);
        ...
        store32(argon_blockhash + ARGON2_PREHASH_DIGEST_LENGTH, 1);
...
void initial_hash(uint8_t *argon_blockhash, argon2_context *context,
                  argon2_type type) {
    ...
    ablake2b_final(&BlakeHash, argon_blockhash, ARGON2_PREHASH_DIGEST_LENGTH);

}

int initialize(argon2_instance_t *instance, argon2_context *context) {
    uint8_t argon_blockhash[ARGON2_PREHASH_SEED_LENGTH];
    ...
    clear_internal_memory(argon_blockhash + ARGON2_PREHASH_DIGEST_LENGTH,
                          ARGON2_PREHASH_SEED_LENGTH -
    ...
    /* Clearing the hash */
    clear_internal_memory(argon_blockhash, ARGON2_PREHASH_SEED_LENGTH);
